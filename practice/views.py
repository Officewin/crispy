...
from .models import Question, UserProgress, Topic


def get_topics(request):
    """Return all available topics."""
    topics = list(Topic.objects.order_by("rank").values("topic_id", "name"))
...
def get_random_questions(request):
    ...
    if topics:
        if all(t.isdigit() for t in topics):
            qs = qs.filter(skillset__topic__topic_id__in=topics)
        else:
